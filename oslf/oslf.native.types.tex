...
Given a category $C = 1 \xrightarrow{f} M$, where 1 is terminal, $C$ has five morphisms: $\{1, M, f, !, f!\}$, where $!\colon M \to 1$ and $f!\colon M \xrightarrow{!} 1 \xrightarrow{f} M.$

\subsubsection{The subobject classifier: $\Omega$.}

To understand the subobject classifier $\Omega$ in the topos $\hat{C} = {\rm Set}^{C^{\rm op}}$, let's consider the behavior on objects and morphisms.

\paragraph{Behavior on objects.}

$\Omega(1)$ is the set of sieves on $1$, the set of subsets of morphisms into $1$ that are closed under precomposition with morphisms in $C$.
$\{1, !\}= \top$
$\{\} = \bot$
We can't have $1$ without $! = 1!$.  We can't have $!$ without $1 = !f$.

$\Omega(M)$ is the set of sieves on $M$, the set of subsets of morphisms into $M$ that are closed under precomposition with morphisms in $C$.
$\{M, f, f!\} = \top$
$\{f, f!\}$
$\{\} = \bot$
...
\begin{remark} The object 0 in the topos
  $\begin{array}{rcl}0:C^{\rm op} &\to& {\rm Set}\\c & \mapsto & \emptyset\end{array}$
\end{remark}

\begin{remark}
The object 1 in the topos
  $\begin{array}{rcl}1:C^{\rm op} &\to& {\rm Set}\\c & \mapsto & 1\end{array}$
\end{remark}
...
%% - n generators g_i: 1 -> M
%% - identity e: 1 -> M
%% - multiplication *: M x M -> M
