...
\begin{mathpar}
  \inferrule* [lab=Rule]{\Gamma_{1} \vdash P_{1} : \mathbf{U}_{1} : \mathbf{X}_{1} \;\ldots\; \Gamma_{n} \vdash P_{n} : \mathbf{U}_{n} : \mathbf{X}_{n}}{\Gamma(\Gamma_{1},\ldots,\Gamma_{n}) \vdash P(P_{1},\ldots,P_{n}) : \mathbf{U}(\mathbf{U}_{1},\ldots,\mathbf{U}_{n}) : \mathbf{X}(\mathbf{X}_{1},\ldots,\mathbf{X}_{n})}
\end{mathpar}

then the denotation of such a rule is

$$\begin{array}{l}
  \meaningof{\mathsf{Rule}} : \meaningof{P_{1}} : \meaningof{\Gamma_{1}} \to \meaningof{\mathbf{U}_{1} : \mathbf{X}_{1}} \times \cdots \times \meaningof{P_{n}} : \meaningof{\Gamma_{n}} \to \meaningof{\mathbf{U}_{n} : \mathbf{X}_{n}} \\\Rightarrow\\ \meaningof{P(P_{1},\ldots,P_{n})} : \meaningof{\Gamma(\Gamma_{1},\ldots,\Gamma_{n})} \to \meaningof{\mathbf{U}(\mathbf{U}_{1},\ldots,\mathbf{U}_{n}) : \mathbf{X}(\mathbf{X}_{1},\ldots,\mathbf{X}_{n})}
\end{array}$$
...
hom-functor of $\mathsf{NT}$. That is, ${P_{1}} : \meaningof{\Gamma_{1}} \to \meaningof{\mathbf{U}_{1} : \mathbf{X}_{1}} = \mathsf{NT}(-_{1},-_{2})$ because barring any further information regarding the value of these meta-variables $\meaningof{P_{1}} : \meaningof{\Gamma_{1}} \to \meaningof{\mathbf{U}_{1} : \mathbf{X}_{1}}$ could be any morphism in $\mathsf{NT}(\meaningof{\Gamma_{1}},\meaningof{\mathbf{U}_{1} : \mathbf{X}_{1}})$; and since neither $\Gamma_{1}$ nor $\mathbf{U}_{1} : \mathbf{X}_{1}$ are specified it could be any morphism in $\mathsf{NT}(-_{1},-_{2})$. Thus, if we let $\mathsf{NT}(-_{1}, -_{2})^n = \mathsf{NT}(-_{1}, -_{2}) \times \cdots \times \mathsf{NT}(-_{1}, -_{2})$ the meaning of an inference rule is a (di-)natural transformation from $\mathsf{NT}(-_{1}, -_{2})^{n} \Rightarrow k \circ \mathsf{NT}(-_{1}, -_{2})^{n}$ where $k$ is the constructor of the consequent of the rule.

\subsubsection{Boundaries}
\begin{mathpar}
  \inferrule* [lab=Axiom] { }{p : \mathbf{U} : \mathbf {X} \vdash p : \mathbf{U} : \mathbf {X}} \\
  \and
...
$$\begin{array}{l}
  \meaningof{\mathsf{Axiom}} : \meaningof{-} \Rightarrow \meaningof{p} : \meaningof{p : \mathbf{U : X}} \to \meaningof{\mathbf{U : X}}
\end{array}$$

We interpret $\meaningof{\mathsf{Id}}$ as $\mathsf{NT}(-_{1},-_{2})$, the hom-functor. We interpret \meaningof{-} as $\mathbf{C_{1}}$, the functor that maps each pair of objects to the one element set. Thus, our transformation looks like:

\begin{mathpar}
  \meaningof{\Vdash p : \mathbf{U} : \mathbf {X} \vdash p : \mathbf{U} : \mathbf {X}} = \mathsf{id} : \mathbf{C_{1}}(-_{1},-_{2}) \to \mathsf{NT}(-_{1},-_{2}) 
\end{mathpar}
