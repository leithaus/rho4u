...
In general, the denotation of inference rules will be a natural or dinatural transformation from functors whose source and target are products of copies of the Grothedieck category $\mathsf{NT}$.
%% 
\begin{mathpar}
  \meaningof{\Vdash p : \mathbf{U} : \mathbf {X} \vdash p : \mathbf{U} : \mathbf {X}} = \mathsf{id} : \mathbf{C_{1}} \to \mathsf{NT}(-,=) 
\end{mathpar}

Let $\mathsf{\meaningof{Id}} = p : \mathbf{U} : \mathbf {X} \vdash p : \mathbf{U} : \mathbf {X}$. From no assumptions we seek to provide the identity morphism on each object, i.e. the components of the dinatural transformation. \mathbf{C_{1}} is the functor that represents no assumptions; it maps each pair of objects to the one element set. 
