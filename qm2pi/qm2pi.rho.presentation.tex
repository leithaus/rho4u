...
While it is standard in calculi such as the rho-calculus to define a
variety of common values such as the natural numbers and booleans in
terms of Church-numeral style encodings, it is equally common to
simply embed values directly into the calculus. Not being
...
values, the naturals, the booleans, the reals, etc as processes. This
...
such as $\prefix{x}{y}5+*y$, and in contexts
$(\prefix{x}{y}5+*y)\mathsf{|}\outputp{x}{1}$ this will evaluate as
expected, producing the process (aka value) $\mathsf{6}$.
...
to the value passed and not the \emph{name} of the value.
...
\begin{mathpar}
  ...
  \and \\
  \inferrule* [lab=process] {} {K_{P} \bc K_{M} \;|\; P\mathsf{|}K_{P}}
\end{mathpar}
