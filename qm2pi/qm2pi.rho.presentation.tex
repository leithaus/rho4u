...
that $0 \bot 0$ and thus $\quotep{0} \bot \quotep{0}$. For example,
let $P_{1} := \prefix{\quotep{0}}{0}{0} \mathsf{|} \outputp{\quotep{0}}{0}$. Then $P_{1} \red \pzero$ because $0 \bot
0$. Suppose now that we set $x_{0}^{-}, x_{0}^{+} := \quotep{0}$. Then, we can write $P_{1}$ as
$\prefix{x_{0}^{-}}{0}{0} \mathsf{|} \outputp{x_{0}^{+}}{0}$. Now, set

\begin{mathpar}
  x_{1}^{-}:= \quotep{(\prefix{x_{0}^{-}}{0}{0})}
  \and
  x_{1}^{+} := \quotep{(\outputp{x_{0}^{+}}{0})}
\end{mathpar}

Then define 
$P_{2} := \prefix{x_{1}^{-}}{0}{0} \mathsf{|} \outputp{x_{1}^{+}}{0}$. Then $P_{2} \red 0$ because $x_{1}^{-} \bot x_{1}^{+}$,
and hence $\prefix{x_{1}^{-}}{0}{0} \bot \outputp{x_{1}^{+}}{0}$.

More generally, set

\begin{mathpar}
  x_{i}^{-} := \quotep{(\prefix{x_{i-1}^{-}}{0}{0})}
  \and
  x_{i}^{+} := \quotep{\outputp{x_{i-1}^{+}}{0}} \\
  \and P_{i} := \prefix{\quotep{x_{i-1}^{-}}}{0}{0} \mathsf{|} \outputp{\quotep{x_{i-1}^{+}}}{0}
\end{mathpar}

Then $P_{i} \red 0$ and hence $x_{i}^{-} \bot x_{i}^{+}$.
