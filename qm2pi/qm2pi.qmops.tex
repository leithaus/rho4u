...
\begin{mathpar}
  \ketp{P} \otimes \ketp{Q} := (P\mathsf{|}Q)\{w \cdot x / x, w \cdot y / y : x \in \freenames{P}, y \in \freenames{Q}, w = \quotep{(P\mathsf{|}Q)}\}
\end{mathpar}

which is consistent with inner product and respects the usual identities

\begin{mathpar}
  (x \cdot \ketp{P}) \otimes \ketp{Q} = \ketp{P} \otimes (x \cdot \ketp{Q})
\end{mathpar}
